...
fn get_embedded_ffmpeg_path() -> Result<PathBuf, Box<dyn std::error::Error>> {
    // 获取应用资源目录
...
fn get_embedded_ffprobe_path() -> Result<PathBuf, Box<dyn std::error::Error>> {
    // 获取应用资源目录
    let resource_path = get_resource_dir()?;
    ...
    #[cfg(target_os = "macos")]
    let ffprobe_path = resource_path.join("bin/macos/ffprobe");

    #[cfg(target_os = "linux")]
    let ffprobe_path = resource_path.join("bin/linux/ffprobe");

    if ffprobe_path.exists() {
        Ok(ffprobe_path)
    } else {
        println!("嵌入的 FFprobe 未找到");
        Err("嵌入的 FFmpeg 未找到".into())
    }
}
fn parse_progress(s: &str) {
    if let Some(t) = s.strip_prefix("time=") {
        println!("进度时间: {}", t.split_whitespace().next().unwrap_or(""));
    }
    if let Some(sz) = s.strip_prefix("size=") {
        println!("进度大小: {}", sz.split_whitespace().next().unwrap_or(""));
    }
}
...
// 获取视频总时长
#[tauri::command]
async fn slice_to_ts(input_path: String, output_path: String) -> Result<ProcessResult, String> {
    // 获取嵌入的 FFmpeg 路径
    let ffmpeg_path = get_embedded_ffmpeg_path().map_err(|e| e.to_string())?;
    // 构建 FFmpeg 命令进行视频切片
...
fn parse_progress_line(line: &str) -> Option<f64> {
    // 找 time= 或 out_time_ms= 都可以，这里简单用 time=
    if line.starts_with("out_time_ms=") {
        let ms: f64 = line.strip_prefix("out_time_ms=")?.parse().ok()?;
        return Some((ms / 1000.0).clamp(0.0, 100.0));
    }
    None
}
